...
import (
	...
	"github.com/insight-chain/inb-go/les/flowcontrol"
	"github.com/insight-chain/inb-go/light"
...
func testChainGen(i int, block *core.BlockGen) {
	...
	case 0:
		...
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), acc1Addr, big.NewInt(10000), params.TxGas, nil, types.Ordinary), signer, testBankKey)
		block.AddTx(tx)
	case 1:
		...
		tx1, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), acc1Addr, big.NewInt(1000), params.TxGas, nil, types.Ordinary), signer, testBankKey)
		tx2, _ := types.SignTx(types.NewTransaction(nonce, acc2Addr, big.NewInt(1000), params.TxGas, nil, types.Ordinary), signer, acc1Key)
		tx3, _ := types.SignTx(types.NewContractCreation(nonce+1, big.NewInt(0), 200000, testContractCode), signer, acc1Key)
		testContractAddr = crypto.CreateAddress(acc1Addr, nonce+1)
		tx4, _ := types.SignTx(types.NewContractCreation(nonce+2, big.NewInt(0), 200000, testEventEmitterCode), signer, acc1Key)
	...
	case 2:
		...
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), testContractAddr, big.NewInt(0), 100000, data, types.Ordinary), signer, testBankKey)
		block.AddTx(tx)
	case 3:
