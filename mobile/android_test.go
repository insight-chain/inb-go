...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "ginb-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/insight-chain/inb-go/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "ginb.aar"), "ginb.aar", os.ModePerm)
