...
func (st *StateTransition) TransitionDb() (ret []byte, usedGas uint64, failed bool, err error, receive *big.Int) {
	...
	if contractCreation {
		...
		ret, st.gas, vmerr, receive = evm.NewCall(sender, st.to(), st.data, st.gas, st.value, st.msg.Types())
	}
	if vmerr != nil {
